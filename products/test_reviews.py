...
class ReviewTests(APITestCase):
 ...
 def test_create_review_once(self):
     res1 = self.client.post("/api/reviews/", {"product":
     self.product.id, "rating": 5, "comment": "Top"}, format="json")
     self.assertEqual(res1.status_code, status.HTTP_201_CREATED, res1.content)
     res2 = self.client.post("/api/reviews/", {"product":
     self.product.id, "rating": 4}, format="json")
 ...
 def test_filter_products_by_min_rating(self):
     ...
     self.assertEqual(res.status_code, status.HTTP_200_OK)
     self.assertTrue(any(p["id"] == self.product.id for p in
     res.data["results"]))